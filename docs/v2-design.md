...
| **4. Verify engine** | Drift check, fixer gaps, JSON reports | `verify` catches an injected drift on the NAS | 🟢 **implemented (2026-07-10)**: `syrvis verify [--smoke] [--json] [--fix]` + `drift.py` (desired-vs-actual) + `remediation.py` (shared fixer dispatch used by both `doctor --fix` and `verify --fix`). Privileged `--fix` reconciles drift (pull + `up -d`) and applies validator fixes; also fixed M1 (SHIM_IP boot drift), M5 (sudo user), L5 (.env leak). 40 tests. **Acceptance criterion still pending**: NAS injected-drift run — needs a full privileged `setup` + `start` on the NAS first (see next-steps prompt) |
| **5. MCP + skills** | MCP server over SSH+`--json`; sudoers enumeration; home-tech operator skills | home-tech session deploys + verifies a Layer 2 service end-to-end via MCP | 🟢 **implemented + red-teamed (2026-07-10, Opus)**: `packages/syrviscore-mcp` — 23 tools, injection-safe SSH, HMAC confirmation, generated sudoers + forced-command shim (`docs/mcp-design.md`). Adversarial security review (`docs/mcp-security-review-2026-07.md`) found + fixed 10 findings; 180 offline tests + CI job. **Remaining**: NAS-side provisioning (dedicated operator user + root-owned sudoers/shim/key — needs a human, see MCP README §Provision) and the home-tech `.mcp.json` + operator skills |
