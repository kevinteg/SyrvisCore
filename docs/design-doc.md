...
└── build-tools/select-docker-versions.py  # image-pin sync helper
...
# generated config.yaml asset that overrides them per-version.
```
...
- Runtime secrets live in `$SYRVIS_HOME/config/.env` (0600, generated by
  `syrvis setup` or applied via a `syrvis-instance` bundle)
- Per-service secrets: the declared `env_file` under `data/<name>/` (0600),
  written via `syrvis secret set` / `syrvis deploy` (stdin-only)
- ACME certificates: `data/traefik/acme.json` (0600, never overwritten)
