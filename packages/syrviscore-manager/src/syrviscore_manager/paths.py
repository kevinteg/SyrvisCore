...
def create_syrvis_wrapper(home: Path) -> Path:
    ...
    The baked ``INSTALL_DIR`` stays the fast path — it is right ~always, and the
    sudoers policy and the shim allowlist match this wrapper by absolute path
    anyway. What is new (incident 2026-08-16) is that it is no longer the ONLY
    path: when the baked home has no ``current`` symlink, the wrapper scans the
    volumes for the install rather than declaring nothing installed.

    That message was the actively harmful part. "No service version installed.
    Run 'syrvisctl install'" is advice that, in the one situation that produces
    it on a working box, would install a SECOND tree beside the intact one and
    make the recovery materially worse — a runbook has to explicitly forbid it.
    The truthful reading of "the wrapper is here but its target is not" is that
    the tree MOVED, so say that, and say where it went.
    """
    bin_dir = home / "bin"
...
# DISCOVERY FALLBACK. Reached only when the baked path has no active version —
# on a healthy box, never. DSM renames a volume root whose name collides with a
# shared folder at the first COLD boot ({package}/ -> {package}_1/), which
# leaves this wrapper (and the sudoers policy, and the shim) pointing at a path
# that no longer exists while the whole install sits intact one name away.
# Exactly ONE candidate is accepted: two would be a genuine ambiguity, and
# guessing between two installs is worse than refusing.
if [ ! -L "$CURRENT_VERSION" ]; then
    FOUND=""
    COUNT=0
    for CAND in /volume[0-9]*/{package} /volume[0-9]*/{package}_[0-9]*; do
        [ -f "$CAND/.syrviscore-manifest.json" ] || continue
        [ -L "$CAND/current" ] || continue
        FOUND="$CAND"
        COUNT=$((COUNT + 1))
    done
    if [ "$COUNT" -eq 1 ]; then
        echo "WARNING: install found at $FOUND, not the baked $INSTALL_DIR" >&2
        echo "WARNING: the tree was moved or renamed - running from $FOUND anyway." >&2
        echo "WARNING: move it back to $INSTALL_DIR: the sudoers policy, the operator" >&2
        echo "WARNING: shim and the rc.d boot hook all still point at the baked path." >&2
        INSTALL_DIR="$FOUND"
        CURRENT_VERSION="${{INSTALL_DIR}}/current"
    elif [ "$COUNT" -gt 1 ]; then
        echo "Error: $COUNT SyrvisCore installs found and the baked path" >&2
        echo "       $INSTALL_DIR is not one of them - refusing to guess." >&2
        echo "       Inspect: ls -d /volume*/{package}*" >&2
        exit 1
    fi
fi
...
if [ -L "$CURRENT_VERSION" ]; then
    ...
    echo "Error: no active service version at $INSTALL_DIR." >&2
    echo "       The version may be installed at a RENAMED path - check" >&2
    echo "       'ls -d /volume*/{package}*' before installing anything." >&2
    echo "       A 'syrviscore_1' sibling means DSM renamed the root at boot;" >&2
    echo "       move it back. Installing again would create a SECOND tree." >&2
    exit 1
fi
""".format(
        home=home, package=PACKAGE_NAME
    )
