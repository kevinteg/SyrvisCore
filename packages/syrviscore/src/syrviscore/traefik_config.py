...
def generate_traefik_static_config() -> str:
    ...
    # Prometheus metrics on the api/internal entrypoint (:8080/metrics) — opt-in via
    # TRAEFIK_METRICS_PROMETHEUS=true in the .env (same os.getenv pattern as the ACME
    # and log settings), so enabling it is config-as-code rather than a NAS-only edit.
    # A scraper (e.g. Prometheus/vmagent) then gets traefik_tls_certs_not_after (TLS
    # cert expiry) + request/entrypoint/router series. Default entryPoint is the
    # internal "traefik" one (:8080), which api.insecure already serves.
    metrics_block = ""
    if os.getenv("TRAEFIK_METRICS_PROMETHEUS", "").strip().lower() == "true":
        metrics_block = (
            "\nmetrics:\n"
            "  prometheus:\n"
