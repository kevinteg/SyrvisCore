...
def generate_traefik_static_config() -> str:
    ...
    # DNS-01 (Cloudflare) is required for names that resolve to a private IP
    # (split-horizon / internal-only): it proves control via a DNS TXT record, so
    # certs issue + renew without the domain being publicly reachable. It kicks in
    # automatically when a Cloudflare DNS token is configured; otherwise HTTP-01.
    if (
        os.getenv("CLOUDFLARE_DNS_API_TOKEN")
        or os.getenv("TRAEFIK_ACME_CHALLENGE", "").lower() == "dns"
    ):
        acme_challenge = """      dnsChallenge:
        provider: cloudflare
        resolvers:
          - "1.1.1.1:53"
          - "1.0.0.1:53\""""
