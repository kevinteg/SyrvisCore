...
registry (syrviscore.seam.registry). This is the G18 guarantee: the enumerated
sudoers boundary and the shim allowlist are derived from the same source the
runtime uses to build argv, so they cannot drift. A drift test asserts the
committed artifacts equal this generator's output.
...
# install (`python .../seam/gen.py ...`). The generator only needs the stdlib,
...
def render_sudoers(cfg: DeployConfig = DEFAULT) -> str:
    lines = [
        ...
        "# GENERATED from syrviscore/seam/registry.py by syrviscore.seam.gen — do not hand-edit.",
...
def render_provision(
    ...
    installs the sudoers policy atomically (no visudo needed — DSM has none), and
    records the true pre-install state of every system file it touches first.
    """
    pubkey = pubkey.strip()
...
# GENERATED by syrviscore.seam.gen for this specific NAS deployment.
#
# Provisions the least-privilege MCP operator account on the Synology NAS:
#   1. create the '{cfg.operator}' user (if missing), give it a real login shell
#      (/bin/sh — DSM defaults to nologin, which can't run the shim), docker access
#   2. install the enumerated NOPASSWD sudoers policy (atomic; DSM has no visudo)
...
write_rollback() {
    ...
    tmp="$ROLLBACK.new"
    {
        ...
        # helper: surgically restore ONE user's login shell in /etc/passwd, so a
        # rollback never full-overwrites passwd and clobbers unrelated changes.
        printf "_setshell() {\n  awk -F: -v u=\"\$1\" -v s=\"\$2\" 'BEGIN{OFS=\":\"} \$1==u{\$7=s} {print}' /etc/passwd > \"/etc/.passwd.rb.\$\$\" && chmod 0644 \"/etc/.passwd.rb.\$\$\" && mv -f \"/etc/.passwd.rb.\$\$\" /etc/passwd || rm -f \"/etc/.passwd.rb.\$\$\"\n}\n"
        while read -r action a b; do
            case "$action" in
                restore)  printf "cp -a '%s' '%s'\n" "$a" "$b" ;;
                remove)   printf "rm -f '%s'\n" "$a" ;;
                setshell) printf "_setshell '%s' '%s'\n" "$a" "$b" ;;
            esac
        done < "$MANIFEST"
...
# --- 1b. operator login shell ---------------------------------------------
# DSM's synouser creates accounts with /sbin/nologin. sshd runs the forced-
# command shim THROUGH the login shell, and nologin refuses to exec anything,
# so every MCP call would fail with a misleading password prompt. Give the
# operator a real shell — set surgically (only this user's 7th passwd field).
STEP="set operator login shell"
CUR_SHELL=$(awk -F: -v u="$OPERATOR" '$1==u{print $7; exit}' /etc/passwd)
[ "$DRYRUN" = 1 ] && [ -z "$CUR_SHELL" ] && CUR_SHELL="/sbin/nologin"
if [ "$CUR_SHELL" = "/bin/sh" ]; then
    say "'$OPERATOR' already has /bin/sh"
elif [ "$DRYRUN" = 1 ]; then
    say "would set '$OPERATOR' login shell to /bin/sh (was '$CUR_SHELL'; nologin can't run the shim)"
    printf '   [dry-run] rewrite /etc/passwd shell of %s -> /bin/sh (atomic) + record surgical rollback\n' "$OPERATOR"
else
    say "setting '$OPERATOR' login shell to /bin/sh (was '$CUR_SHELL'; nologin can't run the shim)"
    # record the TRUE original shell ONCE, for a surgical rollback
    if [ ! -e "$ORIG_DIR/.captured-shell" ]; then
        mkdir -p "$ORIG_DIR" || die "mkdir failed"
        : > "$ORIG_DIR/.captured-shell"
        printf 'setshell %s %s\n' "$OPERATOR" "$CUR_SHELL" >> "$MANIFEST"
        write_rollback
    fi
    PW_NEW="/etc/.passwd.syrvis.$$"   # same dir as /etc/passwd -> atomic rename
    awk -F: -v u="$OPERATOR" 'BEGIN{OFS=":"} $1==u{$7="/bin/sh"} {print}' /etc/passwd > "$PW_NEW" \
        || { rm -f "$PW_NEW"; die "could not rewrite /etc/passwd"; }
    # refuse to install unless the record count is unchanged and the edit took
    if [ "$(awk 'END{print NR}' /etc/passwd)" != "$(awk 'END{print NR}' "$PW_NEW")" ] \
       || ! grep -Eq "^$OPERATOR:.*:/bin/sh$" "$PW_NEW"; then
        rm -f "$PW_NEW"; die "refusing to install /etc/passwd (safety check failed)"
    fi
    chmod 0644 "$PW_NEW" || { rm -f "$PW_NEW"; die "chmod failed"; }
    mv -f "$PW_NEW" /etc/passwd || { rm -f "$PW_NEW"; die "could not install /etc/passwd"; }
    say "login shell set to /bin/sh"
fi
...
Verify the boundary from your Mac (KEY = your operator private key, e.g.
~/.ssh/syrvis_mcp_ed25519; NAS = this box's address). All but 'sudo -l' DENIED:
  ssh -i KEY $OPERATOR@NAS 'id'                # rejected by the forced-command shim
  ssh -i KEY $OPERATOR@NAS 'sudo -n /bin/sh'   # denied by the sudoers policy
  ssh -i KEY $OPERATOR@NAS 'sudo -l'           # lists ONLY the enumerated commands
...
def main(argv) -> int:
    parser = argparse.ArgumentParser(prog="syrviscore.seam.gen", description=__doc__)
