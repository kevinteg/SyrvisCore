...
# existing is that ``$SYRVIS_HOME/config/.env`` — where NTFY_URL normally lives —
# is inside the very tree the alarm has to be able to alarm ABOUT. Module-level
# so tests can repoint it (like NETWORK_SCRIPTS_DIR).
...
# alarming else-branch fed by the rootfs boot-env cache.
BOOT_HOOK_CONTRACT = 2
...
def render_boot_env_cache(ntfy_url: str) -> str:
    """Render the ROOTFS boot-env cache (``/usr/local/etc/syrviscore-boot.env``).

    The boot hook's alarm path must not depend on anything under SYRVIS_HOME —
    see :data:`BOOT_ENV_PATH`. This is deliberately a one-key file: the ONLY
    thing the rootfs needs is somewhere to shout when the tree is gone. Sourced
    by ``/bin/sh``, so the value is single-quoted (an ntfy URL cannot contain a
    single quote; one is stripped defensively anyway). Pure.
    """
    safe = str(ntfy_url or "").replace("'", "")
    return (
        ...
        "# (SYRVIS_HOME renamed/absent) can still page. Rootfs-resident on purpose.\n"
        "NTFY_URL='{}'\n".format(safe)
    )
...
def read_env_value(env_path: Path, key: str) -> str:
    ...
    Deliberately dumb (no shell, no sourcing): the boot-env cache only needs one
    scalar, and this runs as root against a 0600 file. Returns "" when the file
    or key is absent/unreadable.
    """
    try:
...
case "$1" in
    start)
        ...
        #    A `<volume>/syrviscore_<N>` that carries an install manifest (the
        ...
        for VOL in /volume[0-9]*; do
            [ -d "$VOL" ] || continue
            for RENAMED in "$VOL"/syrviscore_[0-9]*; do
                [ -d "$RENAMED" ] || continue
                if [ ! -f "$RENAMED/.syrviscore-manifest.json" ] && [ ! -d "$RENAMED/apps" ]; then
                    continue
                fi
                TARGET="$VOL/syrviscore"
                if [ -e "$TARGET" ]; then
...
class DsmOperations(SystemOperations):
    ...
    def _ensure_boot_env_cache(self, install_dir: Path) -> str:
        ...
        Runs on EVERY boot-script ensure so the cached NTFY_URL tracks the
        instance config. It is written 0600 on the rootfs because that is the
        whole point (see :data:`BOOT_ENV_PATH`): the boot hook's alarm must
        survive SYRVIS_HOME being renamed, unreadable or gone.

        Never fails the boot-script install — a missing NTFY_URL just means the
        hook logs instead of paging, which is strictly better than refusing to
        install the hook. Returns a suffix for the caller's message ("" when
        nothing changed, so an idempotent pass stays quiet).
        """
        ...
        try:
            _, state = _write_script_if_changed(
                BOOT_ENV_PATH, render_boot_env_cache(ntfy_url), 0o600
            )
        except (OSError, PermissionError) as e:
