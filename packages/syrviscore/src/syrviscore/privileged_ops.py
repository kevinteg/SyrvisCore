...
case "$1" in
    start)
        ...
        ;;
    stop)
        # Cleanup macvlan shim on shutdown (optional)
...
"""
        ...
        try:
            ...
            # Check if boot script already exists and is correct
            if boot_script_path.exists():
                existing_content = boot_script_path.read_text()
                if str(startup_script) in existing_content:
                    return True, f"Boot script already configured: {boot_script_path}"
