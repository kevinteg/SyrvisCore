...
from syrviscore.paths import read_env_value  # noqa: F401 - re-exported (stable name)
...
# The rc.d hook DSM runs at boot. Module-level so the writer, the validator and
# the manager-side `syrvisctl doctor` all name the same path.
BOOT_SCRIPT_PATH = Path("/usr/local/etc/rc.d/S99syrviscore.sh")

# ROOTFS cache of the handful of values the boot hook needs when SYRVIS_HOME is
# UNREACHABLE (incident 2026-08-16). Written next to the boot hook by
# ``ensure_boot_script``; read by the S99 ``start`` case. Its whole reason for
# existing is that ``$SYRVIS_HOME/config/.env`` — where NTFY_URL and the
# apps-root segment live — is inside the very tree the hook has to be able to
# reclaim and alarm ABOUT. Module-level so tests can repoint it (like
# NETWORK_SCRIPTS_DIR).
BOOT_ENV_PATH = Path("/usr/local/etc/syrviscore-boot.env")

# CROSS-PACKAGE CONTRACT MARKER for the rc.d hook, rendered into the file itself.
#
# `SystemValidator.check_boot_script` detects drift by comparing the whole
# deployed file against `render_boot_script` — which it can only do when
# SYRVIS_HOME resolves, i.e. never in the failure this hook exists for. The
# MANAGER (`syrvisctl doctor`) has to answer "is the deployed hook current?" from
# the rootfs alone, and it deliberately does not import this package, so it reads
# this integer out of the deployed text instead.
#
# BUMP IT whenever the hook gains a capability an operator would be wrong to
# assume is present. 1 = the design/28 graceful-shutdown stop case. 2 = the
# 2026-08-16 start case: inlined seam heal, collision reclaim guard, and an
# alarming else-branch fed by the rootfs boot-env cache. 3 = the reclaim guard
# also covers the CONFIGURED apps-root segment (SYRVIS_APPS_ROOT_NAME), so the
# 2026-08-16 `syrviscore` -> `syrviscore-apps` share rename is guarded on the
...
def render_boot_env_cache(ntfy_url: str, apps_root_name: Optional[str] = None) -> str:
    """Render the ROOTFS boot-env cache (``/usr/local/etc/syrviscore-boot.env``).

    The boot hook's alarm and reclaim paths must not depend on anything under
    SYRVIS_HOME — see :data:`BOOT_ENV_PATH`. Two keys, both facts the rootfs
    needs when the tree is unreachable: somewhere to shout (``NTFY_URL``), and
    WHICH volume-root directory names are ours to reclaim
    (``SYRVIS_APPS_ROOT_NAME``). Sourced by ``/bin/sh``, so values are
    single-quoted (neither an ntfy URL nor a path segment can contain a single
    quote; one is stripped defensively anyway). Pure.
    """
    safe_url = str(ntfy_url or "").replace("'", "")
    safe_seg = str(apps_root_name or paths.PACKAGE_NAME).replace("'", "")
    return (
        "# SyrvisCore boot-hook cache — auto-generated by syrvis setup, do not edit.\n"
        "# Read by /usr/local/etc/rc.d/S99syrviscore.sh so a decapitated install\n"
        "# (SYRVIS_HOME renamed/absent) can still page — and so the reclaim guard\n"
        "# knows the configured apps-root segment. Rootfs-resident on purpose.\n"
        "NTFY_URL='{}'\n"
        "SYRVIS_APPS_ROOT_NAME='{}'\n".format(safe_url, safe_seg)
    )


def read_apps_root_name(install_dir: Path) -> str:
    """The instance's apps-root segment, read from ``<install_dir>/config/.env``.

    The render-time half of :func:`syrviscore.paths.get_apps_root_name`, used by
    the boot-hook writer AND by the content-aware validator that re-renders the
    hook to compare it — they must agree or every verify would report drift.
    Never raises: a malformed or unreadable value falls back to the default,
    because refusing to install a boot hook over a bad config value would be
    strictly worse than installing one that guards only the default name.
    """
    try:
        return paths.get_apps_root_name(Path(install_dir))
    except Exception:  # noqa: BLE001 - invalid/unreadable config: guard the default
        return paths.PACKAGE_NAME


def render_boot_script(install_dir: Path, apps_root_name: Optional[str] = None) -> str:
    ...
    EVERYTHING in the ``start)`` case is written to work with NOTHING readable
    under ``install_dir`` (incident 2026-08-16). A DSM shared folder that
    collides with the platform's own directory name makes the next COLD boot
    rename the volume roots — ``/volumeN/syrviscore`` -> ``syrviscore_1`` — and
    every control-plane path is absolute into that tree, so one rename took out
    the wrapper, the operator seam, the cron self-heal AND the volume-resident
    ``syrvis-startup.sh`` (which holds the seam heal and the boot-failure ntfy)
    simultaneously. Nothing alarmed, because the alarm was stored inside the
    thing it was supposed to alarm about. This file lives on the DSM rootfs: it
    is the one healer the rename cannot reach, so the seam heal, the reclaim
    guard and the failure alarm are INLINED here rather than trampolined into.

    ``apps_root_name`` is the configured ``SYRVIS_APPS_ROOT_NAME`` segment; the
    reclaim guard scans BOTH it and :data:`syrviscore.paths.PACKAGE_NAME`, so the
    2026-08-16 ``syrviscore`` -> ``syrviscore-apps`` share rename is guarded from
    the moment it is configured. Defaults to the package name so the function
    stays pure (no I/O) — callers pass :func:`read_apps_root_name`.
    """
...
# Auto-generated by syrvis setup - do not edit manually
# boot-hook-contract: {BOOT_HOOK_CONTRACT}
#
# ROOTFS-RESIDENT BY DESIGN: nothing in the start case below may depend on
# {install_dir} being readable — that tree is exactly what a DSM boot-time
# share-name collision renames out from under us (incident 2026-08-16).

BOOT_ENV="{BOOT_ENV_PATH}"

boot_log() {{
    echo "S99syrviscore: $1"
    logger -t S99syrviscore "$1" 2>/dev/null || true
}}

# Direct-POST ntfy read from the ROOTFS cache — never from
# {install_dir}/config/.env, which is inside the tree that can vanish.
# Inert and silent when the cache is absent or carries no NTFY_URL.
boot_alarm() {{
    _title="$1"
    _body="$2"
    _prio="${{3:-urgent}}"
...
case "$1" in
    start)
        # 1. SEAM SHELL HEAL — inlined, needs nothing but /etc/passwd. DSM
        #    regenerates /etc/passwd on EVERY boot, resetting the seam accounts
        #    to /sbin/nologin. The other two copies of this heal (the
        #    volume-resident syrvis-startup.sh and jobs/seam-selfheal) both
        #    resolve through SYRVIS_HOME, so they are unreachable in precisely
        #    the failure where the seam is the only remaining way in. Runs FIRST,
        ...
        #    the real name is free (absent, or an empty scaffold a reconcile
        #    made). A NON-EMPTY impostor is never touched: that needs a human
        #    who can tell a share from a scaffold, so refuse loudly and page.
        #
        #    TWO names are watched: the install-root name (`{package_name}`,
        #    never configurable) and the configured apps-root segment baked in
        #    below. The rootfs cache overrides the baked default, so a hook that
        #    predates a segment change still guards the current one.
        APPS_ROOT_NAME="{apps_root}"
        if [ -f "$BOOT_ENV" ]; then
            SYRVIS_APPS_ROOT_NAME=""
            . "$BOOT_ENV" 2>/dev/null || true
            if [ -n "$SYRVIS_APPS_ROOT_NAME" ]; then
                APPS_ROOT_NAME="$SYRVIS_APPS_ROOT_NAME"
            fi
        fi
        RECLAIM_SEEN=""
        for VOL in /volume[0-9]*; do
            [ -d "$VOL" ] || continue
            for RENAMED in "$VOL"/{package_name}_[0-9]* "$VOL"/"$APPS_ROOT_NAME"_[0-9]*; do
                [ -d "$RENAMED" ] || continue
                # The two globs are the SAME word on a default install; never
                # act on (or page about) one directory twice.
                case " $RECLAIM_SEEN " in *" $RENAMED "*) continue ;; esac
                RECLAIM_SEEN="$RECLAIM_SEEN $RENAMED"
                if [ ! -f "$RENAMED/.syrviscore-manifest.json" ] && [ ! -d "$RENAMED/apps" ]; then
                    continue
                fi
                # Strip DSM's `_<N>` suffix to recover the name it renamed FROM,
                # so one loop serves both watched names.
                RBASE=$(basename "$RENAMED")
                TARGET="$VOL/${{RBASE%_[0-9]*}}"
                if [ -e "$TARGET" ]; then
                    if [ -d "$TARGET" ] && [ -z "$(ls -A "$TARGET" 2>/dev/null)" ]; then
                        rmdir "$TARGET" 2>/dev/null || true
                    else
                        boot_log "REFUSING reclaim: $RENAMED looks like a renamed platform root but $TARGET exists and is NOT empty"
                        boot_alarm "SyrvisCore: platform root NOT reclaimed" \\
                            "$RENAMED is a renamed SyrvisCore root, but $TARGET already exists and is not empty. The homebase is running against the WRONG tree (or not at all). Inspect both, then mv by hand." \\
                            urgent
                        continue
                    fi
                fi
                if mv "$RENAMED" "$TARGET"; then
                    boot_log "reclaimed $RENAMED -> $TARGET (DSM boot-time share-name collision)"
                    boot_alarm "SyrvisCore: reclaimed a renamed platform root" \\
                        "DSM renamed $TARGET to $RENAMED at boot (share-name collision). The boot hook renamed it back. Rename the colliding DSM shared folder before the next cold boot." \\
                        default
                else
                    boot_log "FAILED to reclaim $RENAMED -> $TARGET"
                    boot_alarm "SyrvisCore: reclaim FAILED" \\
                        "Could not mv $RENAMED back to $TARGET. The platform, the operator seam and both self-healers resolve through $TARGET and are DOWN." \\
                        urgent
                fi
            done
        done
        ...
        if [ -x "{startup_script}" ]; then
            ...
            boot_log "startup script missing or not executable at {startup_script} — the homebase will NOT auto-resume"
            boot_alarm "SyrvisCore boot hook: startup script MISSING" \\
                "{startup_script} is absent or not executable after boot, so nothing resumed. Most likely SYRVIS_HOME was renamed (check: ls -d /volume*/syrviscore*) — NEVER run 'syrvisctl install' to fix this." \\
                urgent
        fi
        ;;
    stop)
...
class DsmOperations(SystemOperations):
    ...
    def _ensure_boot_env_cache(self, install_dir: Path) -> str:
        """Refresh the ROOTFS boot-env cache from ``config/.env`` (best-effort).

        Runs on EVERY boot-script ensure so the cached NTFY_URL *and apps-root
        segment* track the instance config. It is written 0600 on the rootfs
        because that is the whole point (see :data:`BOOT_ENV_PATH`): the boot
        hook's alarm and its reclaim guard must survive SYRVIS_HOME being
        renamed, unreadable or gone.

        Written even with no NTFY_URL — the segment alone is worth caching, and
        it is what lets ``syrvisctl doctor`` classify a renamed apps root from
        the rootfs alone. Never fails the boot-script install: a missing NTFY_URL
        just means the hook logs instead of paging, which is strictly better than
        refusing to install the hook. Returns a suffix for the caller's message
        ("" when nothing changed, so an idempotent pass stays quiet).
        """
        ntfy_url = read_env_value(install_dir / "config" / ".env", "NTFY_URL")
        try:
            _, state = _write_script_if_changed(
                BOOT_ENV_PATH,
                render_boot_env_cache(ntfy_url, read_apps_root_name(install_dir)),
                0o600,
            )
        except (OSError, PermissionError) as e:
            return f" (boot-env cache not written: {e})"
        if not ntfy_url:
            return " (boot-env cache: no NTFY_URL in config/.env — the hook will log, not page)"
        return "" if state == "unchanged" else f" (boot-env cache {state})"
    ...
    def ensure_boot_script(self, install_dir: Path) -> Tuple[bool, str]:
        ...
        Also refreshes the rootfs boot-env cache the hook's alarm reads — the
        two are installed together because a hook that can page is the point.
        """
        boot_script_path = BOOT_SCRIPT_PATH
        script_content = render_boot_script(install_dir, read_apps_root_name(install_dir))
        ...
        cache = self._ensure_boot_env_cache(install_dir)

        if state == "created":
            return True, f"Boot script created: {boot_script_path}{cache}"
        if state == "updated":
            return True, f"Boot script updated (content drift): {boot_script_path}{cache}"
        return True, f"Boot script already current: {boot_script_path}{cache}"
